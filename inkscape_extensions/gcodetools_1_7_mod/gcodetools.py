...
G28
G1 Z1.0 F200
G1 X5.0 Y7.0 F9000.0
""",
'footer': """
M 400
M 81
G1 X0.0000 Y0.0000 F9000.0
M84
"""
}
...
################################################################################
	def generate_gcode(self, curve, layer, depth):
		...
		lg, zs, f =  '\nG00', self.options.Zsafe, " F%f"%tool['feed'] 
		current_a = 0
		laser_z = 0.0

		penetration_feed = " F%s"%tool['penetration feed'] 
		passing_feed = " F%s"%tool['passing feed']

		go_to_safe_distance = "\nM 400\nM 81\nG00" + c([None,None,zs]) + penetration_feed + "\n" 
		for i in range(1,len(curve)):
		#	Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
			s, si = curve[i-1], curve[i]
			feed = f if lg not in ['G01','G02','G03'] else ''
			if s[1]	== 'move':
				g += go_to_safe_distance + "G00" + c(si[0]) + passing_feed + "\n" + tool['gcode before path'] + "\n"
				lg = 'G00'
			elif s[1] == 'end':
				...
				if lg=="G00": g += "G01" + c([None,None,s[5][0]+laser_z]) + penetration_feed + "\nM 400" + "\nM 80\n"	
				...
				if (r[0]**2 + r[1]**2)>self.options.min_arc_radius**2:
					...
					if abs(r1.mag()-r2.mag()) < 0.001 :
						...
						g += ("G02" if s[3]<0 else "G03") + c(si[0]+[ s[5][1]+laser_z, (s[2][0]-s[0][0]),(s[2][1]-s[0][1])  ]) + feed + axis4 + "\n"
					else:
						r = (r1.mag()+r2.mag())/2
						#g += ("G02" if s[3]<0 else "G03") + c(si[0]+[s[5][1]+depth]) + " R%f" % (r) + feed  + axis4 + "\n"
						g += ("G02" if s[3]<0 else "G03") + c(si[0]+[s[5][1]+laser_z]) + " R%f" % (r) + feed  + axis4 + "\n"
					lg = 'G02'
				else:
					if tool['4th axis meaning'] == "tangent knife" : 
...
################################################################################
	def check_tools_and_op(self):
		if len(self.selected)<=0 :
			self.error(_("Selection is empty! Will compute whole drawing."),"selection_is_empty_will_compute_drawing")
			paths = self.paths
		else :
			paths = self.selected_paths
		#	Set group
